# These functions are derivates of the 'fuzzyjoin' R package which was
# unfortunately archived from CRAN: https://github.com/dgrtwo/fuzzyjoin

# nocov start
globalVariables(c("i", "key", "value", "name", "newname"))
...
fuzzy_join <- function(x, y, by = NULL, match_fun = NULL,
  ...
  if (!is.null(match_fun)) {
    ...
    matches <- dplyr::bind_rows(lapply(seq_along(by$x), function(i) {
      ...
      x_rep <- unlist(
        purrr::map2(x_indices_l, yls, function(x, y) rep(x, each = y))
      )
      y_rep <- unlist(
        purrr::map2(y_indices_l, xls, function(y, x) rep(y, x))
      )
